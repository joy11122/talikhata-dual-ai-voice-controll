...
export function phoneticKey(input:string){return normalizeVoiceText(input).toLowerCase().replace(/[আঅা]/g,'a').replace(/[ইঈিী]/g,'i').replace(/[উঊুূ]/g,'u').replace(/[এে]/g,'e').replace(/[ওো]/g,'o').replace(/[য়য]/g,'y').replace(/[শষস]/g,'s').replace(/[জঝ]/g,'j').replace(/[চছ]/g,'c').replace(/[টঠ]/g,'t').replace(/[ডঢ]/g,'d').replace(/[তথ]/g,'t').replace(/[দধ]/g,'d').replace(/[পফ]/g,'p').replace(/[বভ]/g,'b').replace(/[কখ]/g,'k').replace(/[গঘ]/g,'g').replace(/[রড়ঢ়]/g,'r').replace(/[ল]/g,'l').replace(/[ম]/g,'m').replace(/[নণ]/g,'n').replace(/[ংঁ]/g,'n').replace(/[হ]/g,'h').replace(/[্]/g,'').replace(/[^a-z0-9ঀ-৿]/g,'').replace(/[cq]/g,'k').replace(/x/g,'ks').replace(/z/g,'j').replace(/v/g,'b').replace(/w/g,'b').replace(/(.)\1+/g,'$1');}
