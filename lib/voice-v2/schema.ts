...
export const VoiceV2Schema = z.object({
  action: z.enum(['CREATE_PARTY','READ_PARTY','LIST_PARTIES','UPDATE_PARTY','DELETE_PARTY','CREATE_PRODUCT','READ_PRODUCT','LIST_PRODUCTS','UPDATE_PRODUCT','DELETE_PRODUCT','CREATE_DUE','RECEIVE_PAYMENT','READ_BALANCE','CREATE_SALE','CREATE_PURCHASE','CREATE_EXPENSE','STOCK_IN','STOCK_OUT','LIST_TRANSACTIONS','DELETE_TRANSACTION']),
...
export const VoiceV2JsonSchema={type:'object',additionalProperties:false,properties:{
 ...
 entityName:{type:['string','null']},targetId:{type:['string','null']},amount:{type:['number','null']},quantity:{type:['number','null']},unit:{type:['string','null']},unitPrice:{type:['number','null']},paidAmount:{type:['number','null']},phone:{type:['string','null']},notes:{type:['string','null']},partyType:{type:['string','null'],enum:['CUSTOMER','SUPPLIER',null]},query:{type:['string','null']},confirmRequired:{type:'boolean'}
},required:['action','entityType','entityName','targetId','amount','quantity','unit','unitPrice','paidAmount','phone','notes','partyType','query','confirmRequired']} as const;